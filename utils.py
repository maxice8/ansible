...
class ShellFact(FactBase):
    ...
    def process(self, output): return output[0] if output else ""

def ensure_secret(secret_name, secret_value):
    """Idempotently creates a podman secret if it doesn't exist."""
    if not secret_value:
        return False
    exists = host.get_fact(ShellFact, f"podman secret exists {secret_name} && echo 'yes' || echo 'no'")
    if exists != "yes":
        server.shell(
            name=f"Store {secret_name} as Podman Secret",
            commands=[f"echo -n '{secret_value}' | podman secret create {secret_name} -"]
        )
        return True
    return False
...
def apply_sysusers(name, content):
    ...
    if changed:
        server.shell(name=f"Apply {name} sysusers", commands=[f"systemd-sysusers /etc/sysusers.d/{name}.conf"])
    return changed
...
def apply_tmpfiles(name, content):
    ...
    if changed:
        server.shell(name=f"Apply {name} tmpfiles", commands=[f"systemd-tmpfiles --create /etc/tmpfiles.d/{name}.conf"])
    return changed
